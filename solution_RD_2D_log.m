% Solver for the 2D reaction-diffusion equation with logistic growth.
% Spatial grid and initial condition can be switched, read the comments.

function [u, r] = solution_RD_2D_log(t_end, gamma, D)  %, rmax, delta_t, delta_r, epsilon)
    ...
    for i = 2:Nr
        ...
        A(i,i)   = 1/delta_t + D/dri * (1/delta_r(i) + 1/delta_r(i-1));
        A(i,i+1) = -D * (1/(dri * delta_r(i)) + d/(2*r(i)*dri)); 

        B(i,i-1) = D * (1/(dri * delta_r(i-1)) - d/(2*r(i)*dri));
        B(i,i)   = 1/delta_t - D/dri * (1/delta_r(i) + 1/delta_r(i-1));
    ...
    % Time-stepping loop
    for n = 1:nt
        u_new = u;  % initial guess for u^{n+1}

        residual = A * u_new - gamma/2 * u_new .* (1 - u_new/K) - B * u - gamma/2 * u .* (1 - u/K);

        % Newton iteration
        lastwarn('');  % Teil des Sicherheitschecks, ob wir ne singuläre Matrix haben
        for iter = 1:10  % stop after 10 to limit comp. time
            jacobian = A - diag(gamma/2 * (1 - 2 * u_new/K));

            delta_u = jacobian \ residual;

            [warnMsg, warnId] = lastwarn;
            if contains(warnId, 'SingularMatrix') || contains(warnId, 'NearlySingularMatrix')
                warning('Matrix is singular or nearly singular. Skipping...');
                u = 0;  % length(u) == 0 kann man easy nach dem Aufruf des Solvers checken
                u_all = NaN;
                final_mass = NaN;
                init_mass = NaN;      
                return;
    ...
    %     int_u_low_r = int_factor * trapz(r_small_range, r_small_range.^(dim-1) .* u(r >= 1 & r <= 2));  % r.^(dim-1)
    % else
    %     int_u_low_r = 0;
    % end
    % 
